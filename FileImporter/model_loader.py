import bpy
from FileImporter.kernel_function import load_model, load_model_from_pose
...
class ImportModel(Operator, ImportHelper):
    ...
    def invoke(self, context, event):
        ...
        if bpy.context.scene.configuration.modelsrc != "":
            self.filepath = bpy.context.scene.configuration.modelsrc
...
class ImportModelfromPoseFile(Operator, ImportHelper):
    ...
    def execute(self, context):
        load_model_from_pose(self.filepath)
        return {'FINISHED'}
    
    def invoke(self, context, event):
        context.window_manager.fileselect_add(self)
        if bpy.context.scene.configuration.modelposesrc != "":
            self.filepath = bpy.context.scene.configuration.modelposesrc
        # Tells Blender to hang on for the slow user input
        return {'RUNNING_MODAL'}
